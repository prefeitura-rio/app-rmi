...
# CF Lookup Feature Tests
echo "=================================================="
echo -e "${BLUE}🏥 Testing CF (Clínica da Família) Lookup Functionality...${NC}"
echo "=================================================="

# Test 120: Get citizen with CF lookup (should trigger background job)
echo -e "${BLUE}Test 120: Get citizen data (should trigger CF lookup)${NC}"
make_request "GET" "/citizen/$CPF" "" "Get Citizen Data (CF Lookup Trigger)"

# Test 121: Wait and verify CF data was populated
echo -e "${BLUE}Test 121: Verify CF data populated (after background processing)${NC}"
echo -e "${BLUE}⏳ Waiting 5 seconds for background CF lookup job to complete...${NC}"
sleep 5  # Wait for background job
make_request "GET" "/citizen/$CPF" "" "Get Citizen Data (CF Data Verification)"

# Test 122: Test address change invalidation
echo -e "${BLUE}Test 122: Update address (should invalidate CF data)${NC}"
cf_address_update="{
...
make_request "PUT" "/citizen/$CPF/address" "$cf_address_update" "Update Address (CF Invalidation)"

# Test 123: Verify new CF lookup triggered
echo -e "${BLUE}Test 123: Verify new CF lookup after address change${NC}"
echo -e "${BLUE}⏳ Waiting 5 seconds for background CF lookup job to complete...${NC}"
sleep 5  # Wait for background job
make_request "GET" "/citizen/$CPF" "" "Get Citizen Data (New CF After Address Change)"

# Test 124: Test citizen wallet includes CF data
echo -e "${BLUE}Test 124: Verify CF data included in wallet response${NC}"
make_request "GET" "/citizen/$CPF/wallet" "" "Get Citizen Wallet (With CF Data)"

# Verification Section for CF Lookup Feature
echo -e "${BLUE}🔍 Verifying CF Lookup Functionality...${NC}"
echo "=================================================="

# Check if CF lookup was triggered and data was populated
if [[ -f "/tmp/api_response_Get_Citizen_Data__CF_Data_Verification_" ]]; then
    cf_verification_response=$(cat "/tmp/api_response_Get_Citizen_Data__CF_Data_Verification_")
    echo -e "${BLUE}📋 CF Lookup Verification Result:${NC}"
    
    if command -v jq >/dev/null 2>&1; then
        # Check if CF data exists in response
        cf_data_exists=$(echo "$cf_verification_response" | jq 'has("clinica_familia")' 2>/dev/null)
        cf_nome_popular=$(echo "$cf_verification_response" | jq -r '.clinica_familia.nome_popular // "null"' 2>/dev/null)
        cf_distance=$(echo "$cf_verification_response" | jq -r '.clinica_familia.distance_meters // "null"' 2>/dev/null)
        cf_source=$(echo "$cf_verification_response" | jq -r '.clinica_familia.lookup_source // "null"' 2>/dev/null)
        
        if [[ "$cf_data_exists" == "true" ]] && [[ "$cf_nome_popular" != "null" ]]; then
            echo -e "${GREEN}✅ CF lookup successful - CF data found in citizen response${NC}"
            echo "  CF Name: $cf_nome_popular"
            echo "  Distance: $cf_distance meters"
            echo "  Source: $cf_source"
            ((TESTS_PASSED++))
            
            # Verify address change triggered new lookup
            if [[ -f "/tmp/api_response_Get_Citizen_Data__New_CF_After_Address_Change_" ]]; then
                new_cf_response=$(cat "/tmp/api_response_Get_Citizen_Data__New_CF_After_Address_Change_")
                new_cf_nome=$(echo "$new_cf_response" | jq -r '.clinica_familia.nome_popular // "null"' 2>/dev/null)
                
                if [[ "$new_cf_nome" != "null" ]]; then
                    echo -e "${GREEN}✅ CF lookup after address change successful${NC}"
                    echo "  New CF Name: $new_cf_nome"
                    ((TESTS_PASSED++))
                    
                    # Check if CF changed (different CFs for different addresses)
                    if [[ "$cf_nome_popular" != "$new_cf_nome" ]]; then
                        echo -e "${GREEN}✅ Address change correctly triggered different CF lookup${NC}"
                        ((TESTS_PASSED++))
                    else
                        echo -e "${YELLOW}⚠️ Same CF found for different addresses (may be normal for nearby locations)${NC}"
                    fi
                else
                    echo -e "${RED}❌ CF lookup after address change failed${NC}"
                    ((TESTS_FAILED++))
                fi
            fi
        else
            echo -e "${YELLOW}⚠️ CF lookup may not have completed yet or no CF found for address${NC}"
            echo -e "${YELLOW}📝 This could be normal if:${NC}"
            echo -e "${YELLOW}   - Address is outside Rio de Janeiro${NC}"
            echo -e "${YELLOW}   - MCP server is unavailable${NC}"
            echo -e "${YELLOW}   - User already has CF data from base records${NC}"
            
            # Check if user already has CF indicator from base data
            cf_indicator=$(echo "$cf_verification_response" | jq -r '.saude.clinica_familia.indicador // false' 2>/dev/null)
            if [[ "$cf_indicator" == "true" ]]; then
                echo -e "${GREEN}✅ User already has CF data from base records (lookup not needed)${NC}"
                ((TESTS_PASSED++))
            fi
        fi
    else
        echo -e "${YELLOW}⚠️ CF lookup verification skipped (jq not available)${NC}"
        if echo "$cf_verification_response" | grep -qE '"clinica_familia"'; then
            echo -e "${GREEN}✅ CF data field found in response${NC}"
            ((TESTS_PASSED++))
        else
            echo -e "${YELLOW}⚠️ No CF data field in response (may be normal)${NC}"
        fi
    fi
else
    echo -e "${RED}❌ CF lookup verification data not available${NC}"
    ((TESTS_FAILED++))
fi

# Verify wallet includes CF data
if [[ -f "/tmp/api_response_Get_Citizen_Wallet__With_CF_Data_" ]]; then
    wallet_cf_response=$(cat "/tmp/api_response_Get_Citizen_Wallet__With_CF_Data_")
    echo -e "${BLUE}📋 Wallet CF Data Verification:${NC}"
    
    if command -v jq >/dev/null 2>&1; then
        wallet_cf_exists=$(echo "$wallet_cf_response" | jq 'has("clinica_familia")' 2>/dev/null)
        if [[ "$wallet_cf_exists" == "true" ]]; then
            echo -e "${GREEN}✅ CF data correctly included in wallet response${NC}"
            ((TESTS_PASSED++))
        else
            echo -e "${YELLOW}⚠️ CF data not in wallet response (may be normal if no CF found)${NC}"
        fi
    else
        if echo "$wallet_cf_response" | grep -qE '"clinica_familia"'; then
            echo -e "${GREEN}✅ CF data field found in wallet response${NC}"
            ((TESTS_PASSED++))
        fi
    fi
fi

echo -e "${GREEN}✅ CF lookup functionality tests completed${NC}"
echo ""
